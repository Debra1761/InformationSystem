...
def main(argv=None):
    ...
    with open('MyModelTraining2optimise.txt', "w") as myfile:           
        myfile.write('statesize,EmbeddingSize,dropout_embedding,dropout_W,dropout_U,mse, accuracy, precision, recall, f1 \n')
            ...
            for dropout_W in [0.2,0]:
                for dropout_U in [0.2,0.3,0.5]:
                    for statesize in [128,256]:
                        ...
                        model.add(Bidirectional(GRU(statesize, dropout_W=dropout_W, dropout_U=dropout_U)))  # try using a GRU instead, for fun   #                     
...
#                        model.summary()
                        ...
                        with open('MyModelTraining2optimise.txt', "a") as myfile:
                            myfile.write(str(statesize)+','+str(EmbeddingSize)+','+str(dropout_embedding)+','+str(dropout_W)+','+str(dropout_U)+','+str(mse)+','+str(accuracy)+ ','+str(precision)+','+str(recall)+','+str(f1)+   ' \n')
